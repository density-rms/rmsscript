...
def apStatsSort(newdate,summaryDict):
    ...
    headers = 'ap,ip,ap_name,uptime,g_client_count,a_client_count,cpuUse,per_memoryUsage,2g_ch_switch_count,5g_ch_switch_count,heartbeatlost_count,heartbeatlost_time,reboot_count,reboot_reason,target_assert_count,target_assert_times,tx_desc_stuck_count,tx_desc_stuck_times,wmi_stuck_count,wmi_lasttime_reported,target_inactive_count,target_inactive_times,beacon_stuck,2g_ch_util_Busy,2g_ch_util_RX,2g_ch_util_TX,2g_ch_util_Total,5g_ch_util_Busy,5g_ch_util_RX,5g_ch_util_TX,5g_ch_util_Total'.split()
...
if __name__ =='__main__':      
    ...
    message_fp.write('DENSITY network hourly report and channel change history are attached\n\n' )
    message_fp.write('Running data for client count and channel changes are available in the following URL:\n')
    ...
    subprocess.call(['%s/hourly_density_email.sh' % working_dir,  "%s" % fname, "%s" % fname1]) 
